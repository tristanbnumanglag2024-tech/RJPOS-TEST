...
export default function LockScreen({ session, onUnlock }: Props) {
  ...
  const verifyPin = async (p: string) => {
    try {
      ...
      const response = await fetch(`${API_BASE}/auth/pos-lock.php`, {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({
          action: "unlock",
      ...
      try {
        ...
        throw new Error("Invalid response from the POS lock service.");
      }

      if (!response.ok || !data.success || data.locked === true) {
        throw new Error(data.message || "Incorrect PIN. Try again.");
      }
      ...
      setError(
        err instanceof Error ? err.message : "Unable to verify PIN."
      );
      setPin("");
      setVerifying(false);
      setUnlocked(false);
    ...
    <div className="min-h-full bg-slate-900 flex flex-col items-center justify-center px-4 py-8 relative overflow-hidden select-none">
      ...
      <div className="relative z-10 flex flex-col items-center gap-6 sm:gap-8 w-full max-w-xs sm:max-w-sm">
        <div className="text-center space-y-1">
          <div className="text-[58px] sm:text-[72px] font-thin text-white leading-none tracking-tight tabular-nums">
            {time}
          </div>
        ...
        >
          <div
            className={`w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 flex items-center justify-center transition-all duration-500 ${
              unlocked
          ...
          >
            {unlocked ? (
              ...
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" className="text-slate-400">
              ...
              return (
                <button
                  ...
                  className={`h-14 sm:h-14 rounded-xl text-base font-medium transition-all duration-100 active:scale-90 disabled:opacity-30 touch-manipulation ${
                    isClear
                      ? "bg-white/5 border border-white/10 text-slate-400 hover:bg-white/10 text-xs font-semibold"
                      : isBack
                        ? "bg-white/5 border border-white/10 text-slate-400 hover:bg-white/10 flex items-center justify-center"
        ...
        <p className="text-xs text-slate-700">
          Enter your 4-digit PIN to unlock this terminal
        </p>
      </div>
